...
class Attention(nn.Module):
    ...
    def forward(self, x, positions=None):
        ...
        # === Compute attention distance if positions are provided ===
        attn_entropy = None
        attn_distance = None
        if positions is not None:
            D = torch.cdist(positions, positions, p=2)  # (B, N_p, N_p), N_p = num_patches
            D = D.unsqueeze(1).expand(B, self.num_heads, N-1, N-1)  # N-1 because exclude cls
            attn_patch = attn[:, :, 1:, 1:]  # exclude cls token from both sides
            attn_distance = (attn_patch * D).sum(dim=-1).mean()
            
            # === entropy ===
            eps = 1e-12
            # compute H = -sum_j p_j log p_j over the key dimension
            ent = -(attn_patch * torch.log(attn_patch + eps)).sum(dim=-1)  # (B, H, N-1)
...
class Block(nn.Module):
    ...
    def forward(self, x, positions=None):
        attn_out, attn_distance, attn_entropy = self.attn(self.norm1(x), positions=positions)
...
class Layer_scale_init_Block(nn.Module):
    ...
    def forward(self, x, positions=None):
        attn_out, attn_distance, attn_entropy = self.attn(self.norm1(x), positions=positions)  # 支持位置
...
class vit_models(nn.Module):
    ...
    with slight modifications
    """
    def __init__(self, img_size=224,  patch_size=16, in_chans=3, num_classes=1000, embed_dim=864, depth=12,
    ...
    def forward_features(self, x):
        ...
        for i , blk in enumerate(self.blocks):
            x, attn_distance, attn_entropy = blk(x, positions = self.positions)
            attn_distances.append(attn_distance)
            attn_entropies.append(attn_entropy)
        
        # output_dir = "/home/admin/don/Gridpe/eval_results/deit_small_patch16_LS_4"
...
@register_model
def deit_small_patch16_LS(pretrained=False, num_heads = 4, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
...
def deit_large_patch16_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
        img_size = img_size, patch_size=16, embed_dim=1056, depth=24, num_heads=8, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=Layer_scale_init_Block, **kwargs)
    if pretrained:
