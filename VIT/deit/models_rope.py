...
class RoPEAttention(Attention):
    ...
    def forward(self, x, freqs_cis, positions=None):
        ...
        # Apply rotary on non-CLS tokens
        ...
        if positions is not None:
            with torch.no_grad():
                # === distance ===
                D = torch.cdist(positions, positions, p=2)           # (B, N-1, N-1)
                D = D.unsqueeze(1).expand(B, self.num_heads, N-1, N-1)
                patch_attn = attn[:, :, 1:, 1:]
                attn_distance = (patch_attn * D).sum(dim=-1).mean()

                # === entropy ===
                eps = 1e-12
                # compute H = -sum_j p_j log p_j over the key dimension
                ent = -(patch_attn * torch.log(patch_attn + eps)).sum(dim=-1)  # (B, H, N-1)
        ...
        if attn_distance is not None:
            return out, attn_distance, attn_entropy
        else:
            return out
...
class RoPE_Layer_scale_init_Block(Layer_scale_init_Block):
    ...
    def forward(self, x, freqs_cis, positions=None):
        attn_out, attn_distance, attn_entropy = self.attn(self.norm1(x), freqs_cis=freqs_cis, positions=positions)
    ...
    def forward_features(self, x):
        ...
        if self.rope_mixed:
            ...
            for i , blk in enumerate(self.blocks):
                x, attn_distance, attn_entropy = blk(x, freqs_cis=freqs_cis[i], positions=self.positions)
                attn_distances.append(attn_distance)
                attn_entropies.append(attn_entropy)
            
            # output_dir = "/home/admin/don/Gridpe/eval_results/rope_mixed_deit_small_patch16_LS_4"
            # if output_dir is not None and dist.get_rank() == 0:
            #     os.makedirs(output_dir, exist_ok=True)
            #     dist_path = os.path.join(output_dir, "attn_distance.txt")
            #     ent_path  = os.path.join(output_dir, "attn_entropy.txt")
            ...
            attn_entropies = []
            for i , blk in enumerate(self.blocks):
                x, attn_distance, attn_entropy = blk(x, freqs_cis=freqs_cis, positions=self.positions)
                attn_distances.append(attn_distance)
                attn_entropies.append(attn_entropy)

            # output_dir = "/home/admin/don/Gridpe/eval_results/rope_axial_deit_small_patch16_LS_4"
            # if output_dir is not None and dist.get_rank() == 0:
            #     os.makedirs(output_dir, exist_ok=True)
            #     dist_path = os.path.join(output_dir, "attn_distance.txt")
            #     ent_path  = os.path.join(output_dir, "attn_entropy.txt")
...
# RoPE-Axial
@register_model
def rope_axial_deit_small_patch16_LS(pretrained=False, num_heads = 4, img_size=224, pretrained_21k = False,  **kwargs):
    model = rope_vit_models(
        img_size = img_size, patch_size=16, embed_dim=384, depth=12, num_heads=num_heads, mlp_ratio=4, qkv_bias=True,
...
@register_model
def rope_axial_deit_base_patch16_LS(pretrained=False, num_heads = 12, img_size=224, pretrained_21k = False,  **kwargs):
    model = rope_vit_models(
        img_size = img_size, patch_size=16, embed_dim=864, depth=12, num_heads=num_heads, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=RoPE_Layer_scale_init_Block, Attention_block=RoPEAttention,
        rope_theta=100.0, rope_mixed=False, **kwargs)
    return model

@register_model
def rope_axial_deit_large_patch16_LS(pretrained=False, num_heads = 8, img_size=224, pretrained_21k = False,  **kwargs):
    model = rope_vit_models(
        img_size = img_size, patch_size=16, embed_dim=1056, depth=24, num_heads=num_heads, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=RoPE_Layer_scale_init_Block, Attention_block=RoPEAttention,
        rope_theta=100.0, rope_mixed=False, **kwargs)
    return model
...
# RoPE-Mixed
@register_model
def rope_mixed_deit_small_patch16_LS(pretrained=False, img_size=224,num_heads = 4, pretrained_21k = False,  **kwargs):
    model = rope_vit_models(
        img_size = img_size, patch_size=16, embed_dim=384, depth=12, num_heads=num_heads, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), block_layers=RoPE_Layer_scale_init_Block, Attention_block=RoPEAttention,
...
@register_model
def rope_mixed_deit_base_patch16_LS(pretrained=False, num_heads = 12, img_size=224, pretrained_21k = False,  **kwargs):
    model = rope_vit_models(
        img_size = img_size, patch_size=16, embed_dim=864, depth=12, num_heads=num_heads, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=RoPE_Layer_scale_init_Block, Attention_block=RoPEAttention,
        rope_theta=10.0, rope_mixed=True, **kwargs)
...
@register_model
def rope_mixed_deit_large_patch16_LS(pretrained=False,num_heads = 8, img_size=224, pretrained_21k = False,  **kwargs):
    model = rope_vit_models(
        img_size = img_size, patch_size=16, embed_dim=1056, depth=24, num_heads=num_heads, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=RoPE_Layer_scale_init_Block, Attention_block=RoPEAttention,
        rope_theta=10.0, rope_mixed=True, **kwargs)
