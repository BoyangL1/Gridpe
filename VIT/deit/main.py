...
def main(args):
    ...
    if args.eval:
        ...
        output_txt = f"/home/admin/don/Gridpe/eval_results/{args.model}_{args.num_heads}"
        if dist.is_initialized():
            rank = dist.get_rank()
        else:
            rank = 0
        if output_txt is not None and rank == 0:
            os.makedirs(output_txt, exist_ok=True)
            dist_path = os.path.join(output_txt, "attn_distance.txt")
            ent_path  = os.path.join(output_txt, "attn_entropy.txt")
            with open(dist_path, "a") as f_dist, open(ent_path, "a") as f_ent:
                f_dist.write(f"Input size: {args.input_size}\n")
                f_ent.write(f"Input size: {args.input_size}\n")
        ...
        with open(output_file, "w") as f:
            ...
            f.write(f"acc1: {acc1:.4f}\n")
            f.write(f"acc5: {acc5:.4f}\n")
