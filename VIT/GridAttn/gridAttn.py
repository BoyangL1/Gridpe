...
class GridCellPositionalEncoding(nn.Module):
    ...
    def _generate_batch_encoding(self, positions):
        ...
        """
        theta_heads = []
        mag = 1 / (10000 ** (2 * self.n * torch.arange(self.num_scales, dtype=torch.float32, device=positions.device)[:, None] / self.num_head_dim))
...
class GridPEAttention(Attention):
    ...
    def forward(self, x, positions):
        ...
        attn = attn.softmax(dim=-1)
        with torch.no_grad():
            D = torch.cdist(positions, positions, p=2)  # (B, N, N)
            D = D.unsqueeze(1).expand(B, self.num_heads, N_patch, N_patch)  # (B, H, N, N)
            attn_patch = attn[:, :, 1:, 1:]  # skip cls row and col
            attn_distance = (attn_patch * D).sum(dim=-1).mean()
            
            eps = 1e-12
            entropy_head_token = -(attn_patch * torch.log(attn_patch + eps))  # (B, H, N_p, N_p)
            # sum over keys → shape (B, H, N_p)
            entropy_head = entropy_head_token.sum(dim=-1)
            # average over tokens and heads → scalar
            attn_entropy = entropy_head.mean()
        attn = self.attn_drop(attn)
